...
- `gitcode-repo` — 来自 [gitcode-cli/skills](https://gitcode.com/gitcode-cli/skills) 项目（`git@gitcode.com:gitcode-cli/skills.git`），可独立安装使用

## 适用人群

- 平台团队、文档团队
- 多仓维护者
- DevOps（跨仓同步 CI 配置、规范模板）
...
| 占位符 | 案例值 | 替换为 |
|---|---|---|
| 源目录 | `Example/cases` | 源仓库中的目录路径 |
| 目标仓库 | `openLiBingNext/openlibing-platform-release` | 目标仓库 |
| 目标目录 | `docs/gitcode-cli-cases` | 目标仓库中的目录路径 |
| 目标分支 | `master` | 目标仓库默认分支 |
| 同步目的 | 案例沉淀 | 同步的业务原因 |
...
- 跨仓库同步文档、规范、模板、CI 配置
- 多仓组织的知识分发
- 不适合：二进制大文件同步（应使用 release 资产或制品仓库）

### 跨平台提醒

- `gitcode repo sync` 内部使用 SSH，需确认 `ssh -T git@gitcode.com` 可用
- 目标目录会被完整覆盖（非增量合并），注意不要覆盖目标仓库的手动修改
...
- 关联：[新成员上手发布平台仓库](./repo-onboarding.md) — 同步后的文档供新成员阅读
- 关联：[发布 openLiBing 发布平台版本](./publish-release.md) — 文档同步可作为 release 的一部分
