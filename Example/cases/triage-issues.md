...
- `gitcode-issue-triage` — 来自 [gitcode-cli/skills](https://gitcode.com/gitcode-cli/skills) 项目（`git@gitcode.com:gitcode-cli/skills.git`），可独立安装使用
...
| Issue 列表 | #1 到 #5 | 当前 open issue 列表 |
| 已有标签 | `enhancement` | 目标仓库现有标签 |

### 适用场景

- 版本规划前批量整理 Issue 队列
- 发现 Issue 和 PR 状态不一致时对齐
- 不适合：issue 数量极大（>50）且没有明确优先级字段的仓库

### 跨平台提醒

- Windows 下 `gitcode issue comment --body-file` 需 UTF-8 编码

### 前置条件

- 对目标仓库有 Issue 读权限（写操作需要写权限）
- 了解目标仓库的标签体系
- （可选）安装 `gitcode-issue-triage` skill
...
- 前置：[向发布平台提交高质量 Issue](./create-issue.md) — 理解单个 Issue 的质量标准
- 关联：[Issue 实现前评审](./issue-pre-review.md) — triage 后对高优先级 Issue 做深度分析
- 关联：[标签体系与里程碑治理](./label-milestone-governance.md) — 标签体系的建立和维护
