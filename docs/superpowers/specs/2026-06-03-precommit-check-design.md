...
| `--install` / `--no-install` | auto | 缺失时自动安装/初始化(受下方安全约束限制) |
| `--yes, -y` | false | 非交互环境下确认执行会修改环境的自动安装/初始化 |
...
5. **可选拉起**:`--run` 时执行 `pre-commit run --all-files`,透传其退出码。
...
2. 有 `pipx` → `pipx install pre-commit`。
3. 有 `python3` → `python3 -m pip install --user pre-commit`;Windows 回退 `python -m pip install --user pre-commit`。
4. 无 Python → **不自动安装 Python**(过于侵入),报错并给出各平台安装指引(brew / apt / 官网)。

安装后重新探测 `pre-commit --version` 确认成功;失败则报错退出。
