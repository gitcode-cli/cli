...
func TestListRunBuildsV8Query(t *testing.T) {
	...
	want := "/api/v8/repos/owner/repo/actions/runner-sets"
	if !strings.HasPrefix(gotPath, want+"?") {
		t.Fatalf("request path = %q, want v8 repo runner-sets prefix", gotPath)
	}
...
func TestListRunJSONOutput(t *testing.T) {
	...
	opts := &ListOptions{
		...
		HttpClient: func() (*http.Client, error) {
			return &http.Client{
				Transport: testutil.NewRoundTripFunc(func(req *http.Request) (*http.Response, error) {
					return listTestResponse(http.StatusOK, `{"total_count":1,"runner_sets":[{"id":"rs1","runner_group_id":"rg1","name":"k8s-1","status":"using","required_labels":[{"label_name":"linux","label_value":"","label_color":""}]}]}`), nil
				}),
			}, nil
		},
		Repository: "owner/repo",
	...
	if !strings.Contains(got, `"name": "k8s-1"`) {
