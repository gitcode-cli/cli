...
func TestListRunBuildsV8Query(t *testing.T) {
	...
	want := "/api/v8/repos/owner/repo/actions/runners/shared-runners"
	if !strings.HasPrefix(gotPath, want+"?") {
		t.Fatalf("request path = %q, want v8 shared-runners prefix", gotPath)
	}
...
func TestListRunJSONOutput(t *testing.T) {
	...
	opts := &ListOptions{
		...
		HttpClient: func() (*http.Client, error) {
			return &http.Client{
				Transport: testutil.NewRoundTripFunc(func(req *http.Request) (*http.Response, error) {
					return listTestResponse(http.StatusOK, `{"total_count":1,"runners":[{"id":"r1","runner_group_id":"rg1","runner_name":"shared-1","name":"host","work_dir":"/tmp","labels":[{"label_name":"self-hosted","label_value":"","label_color":""}]}]}`), nil
				}),
			}, nil
		},
		Repository: "owner/repo",
	...
	if !strings.Contains(got, `"runner_name": "shared-1"`) {
...
func TestListRunEmptyResultsText(t *testing.T) {
	...
	got := strings.TrimSpace(out.String())
	if got != "No shared runners found" {
		t.Fatalf("empty text = %q, want 'No shared runners found'", got)
	}
}
...
func TestListRunTextOutput(t *testing.T) {
	...
	opts := &ListOptions{
		...
		HttpClient: func() (*http.Client, error) {
			return &http.Client{
				Transport: testutil.NewRoundTripFunc(func(req *http.Request) (*http.Response, error) {
					return listTestResponse(http.StatusOK, `{"total_count":1,"runners":[{"id":"r1","runner_name":"shared-1","labels":[{"label_name":"linux"}]}]}`), nil
				}),
			}, nil
		},
		Repository: "owner/repo",
