...
- Windows npm bootstrap 仅在用户显式执行 `install` 时默认将已校验的目标目录置于当前 User PATH 前面并去重；必须提供 `--no-modify-path` 退出，不得修改 Machine PATH、请求提权、删除其他目录或覆盖其他渠道入口。持久化通过固定 PowerShell 代码、独立环境值和最小环境执行，不得把目标路径或既有 PATH 拼接进可执行脚本，也不得继承任何凭证或 npm registry/auth 配置
- Windows bootstrap 修改持久 User PATH 后必须用中文明确说明 npx 子进程无法刷新父 PowerShell，给出可复制的当前窗口 `$env:Path` 命令、关闭全部 PowerShell/Windows Terminal 后重开的替代方式和 `gitcode version` 验证步骤；持久化失败必须显式报告并提供手工命令，不得伪装成功
