...
优先使用 [docs/REGRESSION.md](../../docs/REGRESSION.md) 和 `go test -tags=system ./tests/system` 执行结构化真实命令系统测试；`./tests/system/run.sh --read` 保留为直接 shell 入口，`./scripts/regression-core.sh` 保留为最小冒烟回归入口。
